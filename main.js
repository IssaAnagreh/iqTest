...
function displayQuestions(num) {
	$('#body').html(''); // our virtual body -excluding nav bar-
	...
	for (var i = 0; i < num; i++) {
		...
		$.each(questions[i].choices,function(j, e){
			choices += '<p class="choice" style="margin-left: 2%"> <input name="'+i+'" type="radio" id="c'+i+j+'" value="'+ e +'" /> <label for="c'+i+j+'">'+e+'</label> </p>';
		});

		var qs = '<div class="question"> 	<p class="question1"> '+questions[i].question+' </p> 	<div id="'+i+'" class="answers"> '+choices+' </div> 		</div> <hr style="margin: 0 auto">';
		
		$('#body').append(qs);
	}

	$('#body').fadeIn(400); // Fading effect for the body
...
function submit(num) {
	...
	$(".btn").click(function() { // .btn is the submit button
		$('html, body').animate( {
...
$("#contact").click(function() {
	$('html, body').animate({
 	scrollTop: $("#con").offset().top
		}, 5000); // Scrolls to the map when clicking on contact us
});
...
//==================================================
var game = '';
var pos = Math.floor(Math.random() * 650);
...
//========================================================
// Fade in // I trasnfered it into the displaying function //
//========================================================
// $(document).ready(function() {
// 	$('#body').fadeIn(1500); // Fading effect for the body
// });
